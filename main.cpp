...
int main()
{
	...
	//===========================
	//===========================
	//NEW CODE INITIALIZING ENEMY
	sf::RectangleShape enemy1(sf::Vector2f(100.0f, 100.0f));
	sf::Texture enemytexture;
	enemytexture.loadFromFile("enemy.jpg");
	enemy1.setTexture(&enemytexture);
	enemy1.setOrigin(50.0f, 50.0f);
	enemy1.setPosition(700.0f, 437.0f);
	bool enemycollide;
	float Enemy_dx = 0.0f;
	//NEW CODE INITIALIZING ENEMY 
	//===========================
	...
	while (window.isOpen())
	{
		...
		//===========================
		//ENEMY DRAW CODE
		enemycollide = enemy1.getGlobalBounds().intersects(player.getGlobalBounds());
		enemy1.move(Enemy_dx, 0);
		if (enemycollide)
		{
			std::cout << "colliding with enemy" << std::endl;
			Player_dy = Player_dy;
			Player_dx = 0;
			player.setPosition(150, 437);
		}
